...
Exit codes are the contract an agent or script switches on:
  ...
  3  no running gateway answered (socket absent, refused, the mint failed -- or no
     AF_UNIX transport at all, where the verb is unavailable and says so)
...
class TestFailureExits:
    ...
    def test_missing_socket_exits_3(
        self, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
    ) -> None:
        # Pin the AF_UNIX-host branch of the message: Windows CPython has no
        # ``socket.AF_UNIX`` and would otherwise take the "unavailable here" branch
        # (covered by the next test), while the transport itself is faked below.
        ...
        err = capsys.readouterr().err
        assert "was not updated" in err
        # A host WITH AF_UNIX: the likely cause is no gateway, the runner-up a
        # sandboxed shell -- so the remedy names both, and Sync for the latter.
        assert "Start the gateway and run this again" in err
        assert "sandboxed away from its socket, use Sync in the dashboard" in err

    def test_without_af_unix_the_verb_says_it_is_unavailable_not_start_the_gateway(
        self, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
    ) -> None:
        """Windows has no ``AF_UNIX``, and the request has no other transport.

        ``toggle_app`` returns ``None`` there whatever the gateway is doing, so
        "start the gateway and run this again" would send the user in a circle. The
        message must say the verb cannot succeed on this platform and hand them the
        dashboard's Sync -- still exit 3, since nothing was changed.
        """
        ...
        assert exc.value.code == 3
        err = capsys.readouterr().err
        assert "no AF_UNIX sockets" in err
        assert "this command is unavailable here: use Sync in the dashboard" in err
        assert "Start the gateway" not in err
