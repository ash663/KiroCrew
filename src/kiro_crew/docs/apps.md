...
5 when the app is not installed (or manages its own updates), and 4 when the
source directory's `app.json` names a different app — refused before anything is
stopped, so a wrong `--source` leaves the running app untouched.
